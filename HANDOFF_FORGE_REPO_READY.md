...
Goal:
Close the W310 review-only source diff packet, verify the exact proposed Electrical Components Distributor source patch is still draft-only, and create a human approval gate that states what must be confirmed before any future `src/contracts/lanePacks.js` mutation is allowed, without applying the mutation, installing the proposed pack, wiring runtime behavior, changing lane resolution behavior, visible UI, story copy, returned-record import, connected submit/refresh/import, endpoint behavior, dataset switching, or runtime authority.

Build:
- Add an archived W310 source diff QA closure map covering:
  - W310 review-only source diff packet
  - W310 diff-readiness acceptance packet
  - W309 source-change readiness blockers
  - W308 review-only candidate proposal and acceptance packet
  - W304-W306 future expansion readiness continuity
  - W247/W251/W252/W255 review expectations
- Add a human approval gate packet that records:
  - real website/category evidence confirmation
  - base-pack comparison approval
  - role/vocabulary/story copy approval
  - N/LLM advisory-only approval
  - W247/W251/W252/W255 QA approval
  - W304-W306 readiness approval
  - source mutation approval status
  - post-install smoke approval status
- Keep the approval gate default decision as `not_approved_yet`.
- Select the next narrow implementation block.
- Prefer a conditional source-change execution prompt that may apply the candidate pack only after explicit human approval is provided in a future user request.
...
- Preserve W310 source diff packet, W309 QA closure/source-change readiness, W308 proposal packet, W307 closure/authoring plan, W306 runtime shape migration, W305 bridge, W304 contract, W303 closure/readiness map, W302 runtime shape migration, W301 bridge, W300 contract, W274 expansion workflow contract, and W277 lane-pack review bridge.
- Preserve W218/W220 wording, fake Open-link blocking, W245 canonical import normalization, W250 lane-aware labels, W262-W310 continuity, N/LLM advisory-only behavior, and uncertainty visibility.
- Keep harnesses, reports, and traces under `archive/`.

Validation:
- Add W311 harness covering:
  - W310 source diff QA closure map exists
  - human approval gate defaults to `not_approved_yet`
  - real website/category evidence and explicit human source approval remain required before source mutation
  - W310 source diff remains represented but unapplied
  - W310 diff-readiness acceptance remains draft-only
  - W308 proposal remains review-only and non-installable
  - W306 runtime shape remains field-compatible with W305 for candidate facts
  - W305 bridge and W304 contract remain available and unchanged
  - W303/W307/W309/W310 closure maps remain available
...
- Run W244 through W311 harnesses, check, and validate.

Output:
- Summary of source diff QA closure, human approval gate, guardrails, and validation.
- Validation results.
