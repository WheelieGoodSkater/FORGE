...
function main() {
  ...
  assertCase(results, 'suitelet-header-logo-proportionate', /width:\s*188px;/.test(userscript) && /max-width:\s*min\(188px, 48vw\);/.test(userscript) && /max-height:\s*58px;/.test(userscript) && /idb-header-meta/.test(userscript) && /idb-bug-button/.test(userscript), 'logo width 188px, max-height 58px, compact SCOUT-style header meta present');
