...
function main() {
  const results = [];
  const hooks = loadHooks({ fetchMessage: 'live fetch disabled in W314 harness' });
  const packageJson = JSON.parse(readRepoFile('package.json'));
  const userscript = read(userscriptPath);
  const lanePacksSource = readRepoFile('src', 'contracts', 'lanePacks.js');
  const report = readArchiveText('reports', 'w314_human_approved_candidate_lane_pack_source_mutation_blocked_noop.md');
  const trace = readArchiveJson('trace_samples', 'w314_human_approved_candidate_lane_pack_source_mutation_blocked_noop_trace.json');
  ...
  const completedResult = completedMotionResult({ prefix: '314' });
  const completedRaw = completedRefreshResponse('runner-w314-motion-001', completedResult);
  const completedShape = hooks.actualAdapterResponseShapeW265(completedRaw, {
    phase: 'refresh',
    runnerTaskId: 'runner-w314-motion-001',
    idempotencyToken: 'motion-w314-token'
  });
  ...
  const w264Flow = hooks.connectedBuildSubmitRefreshImportW264(state, context.lane, context.page, context.recommendation, {
    ...
    executePoll: true,
    finishBuild: true,
    submitTransport: () => submitResponse('runner-w314-motion-001', 'motion-w314-token'),
    pollTransport: () => completedRaw
  });

  assertCase(results, 'w313-approval-intake-inspected-and-blocked-noop-path-selected',
    trace.schema === 'forge.w314.human-approved-candidate-lane-pack-source-mutation.trace.v1' &&
      trace.status === 'blocked_noop_explicit_approval_missing' &&
      trace.pathTaken === 'blocked_noop' &&
      trace.approvalIntakeInspection.w313Decision === 'approval_not_provided' &&
      trace.approvalIntakeInspection.requiredFieldsProvided === false &&
      trace.approvalIntakeInspection.explicitApprovalInW314Request === false &&
      trace.approvalIntakeInspection.approvalMissingOrAmbiguous === true &&
      w313Trace.approvalIntakeDecision === 'approval_not_provided',
    JSON.stringify(trace.approvalIntakeInspection));

  assertCase(results, 'blocked-noop-packet-records-no-source-mutation',
    /W314 Human-Approved Candidate Lane Pack Source Mutation Blocked No-Op/.test(report) &&
  ...
  assertCase(results, 'source-pack-schema-valid-and-existing-lanes-unchanged',
  ...
  assertCase(results, 'w312-w311-w310-continuity-remains-available',
  ...
  assertCase(results, 'returned-records-labels-open-links-and-review-run-copy-unchanged',
    normalizedImport.visibleRecords.some((record) =>
      ...
      /netsuite\.com\/app\/common\/item\/item\.nl\?id=31403/.test(record.supportedOpenUrl)
    ) &&
  ...
  assertCase(results, 'normal-consultant-ui-hides-endpoint-profile-raw-admin-diagnostics',
    !/script=6702|deploy=2|runner-w314-motion-001|finalGeneratedNamesJson|stack trace|schema/i.test(html) &&
  ...
  assertCase(results, 'w314-report-trace-harness-and-check-registration-present',
    packageJson.scripts['harness:human-approved-candidate-lane-pack-source-mutation-w314'] &&
      packageJson.scripts.check.includes('run_w314_human_approved_candidate_lane_pack_source_mutation_harness.js') &&
      trace.nextRecommendedBlock === 'W315: Final Human Approval Capture For Candidate Source Mutation',
    packageJson.scripts['harness:human-approved-candidate-lane-pack-source-mutation-w314'] || '');

  printResults('W314 human-approved candidate lane pack source mutation harness', results);
}
