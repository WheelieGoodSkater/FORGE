...
function fixtureState(hooks, config) {
  ...
  state.dccFinalNamingResult = {
    ...
    importedAt: '2026-06-01T15:00:00.000Z',
...
const fixtures = {
  willow: (hooks) => fixtureState(hooks, {
    laneId: 'food_beverage',
    source: 'fixture_story_layer_w375_no_live_smoke',
    ...
    records: [
      ...
      openRecord('sales_order', 'Sales Order', 'SO-W375 Willow Creek Seasonal Sauce Promotion', '7202', 'https://td3021666.app.netsuite.com/app/accounting/transactions/salesord.nl?id=7202'),
  ...
  northstar: (hooks) => fixtureState(hooks, {
    ...
    intake: {
      ...
      scObjective: 'Prepare a fixture-first Medical/Dental Supply demo story around clinic supply availability.',
      decisionCriteria: 'Show clinic supply availability without food, dealer, apparel, or service language.'
    },
    records: [
  ...
  bayview: (hooks) => fixtureState(hooks, {
  ...
  harbor: (hooks) => fixtureState(hooks, {
    ...
    intake: {
      ...
      scObjective: 'Prepare a fixture-first Apparel & Accessories story.',
      decisionCriteria: 'Show style/SKU matrix without dealer, service, medical, or food language.'
    },
    records: [
...
function main() {
  ...
  const willow = scenarioFromState(hooks, fixtures.willow(hooks), 'Willow Creek Food/Beverage');
  const northstar = scenarioFromState(hooks, fixtures.northstar(hooks), 'Northstar Medical/Dental');
  const bayview = scenarioFromState(hooks, fixtures.bayview(hooks), 'Bayview Parts/Service');
  const harbor = scenarioFromState(hooks, fixtures.harbor(hooks), 'Harbor Apparel/Retail');
  const baselines = [
  ...
  assertCase(results, 'w375-current-drawer-marker-advanced',
    hooks.drawerDisplayVersionW346() === 'Drawer 1.0.21 / W375' &&
      /@version\s+1\.0\.21/.test(userscript) &&
      userscript.includes("const CURRENT_UX_BLOCK_W346 = 'W375';"),
    hooks.drawerDisplayVersionW346());

  assertCase(results, 'w375-story-contract-consistency',
  ...
  assertCase(results, 'w375-helper-consolidation-safety',
    /function renderW375StoryContractLens/.test(userscript) &&
      /function renderW375StoryContractProofPath/.test(userscript) &&
      activeStoryScenarios.every((scenario) => /W375 shared story renderer/.test(scenario.valueHtml + scenario.runHtml)) &&
      activeStoryScenarios.every((scenario) => /idb-w375-story-contract-lens-detail/.test(scenario.valueHtml)) &&
      activeStoryScenarios.every((scenario) => /idb-w375-story-contract-proof-detail/.test(scenario.runHtml)),
    willow.valueHtml.slice(0, 3600) + willow.runHtml.slice(0, 3600));

  assertCase(results, 'w375-first-read-lanes-remain-distinct',
    /ingredient|packaging|case pack|batch|line schedule|QA|lot|finished-good|ship/i.test(firstReadText(willow)) &&
      /clinic|dental|substitute|multi-location|backorder|warranty/i.test(firstReadText(northstar)) &&
    ...
    firstReadText(willow).slice(0, 2200));

  assertCase(results, 'w375-cross-lane-anti-leak-wording',
    !/dealer allocation|supplier portals|channel fulfillment|style\/color\/size variants|store\/ecommerce promise|work order dispatch|technician truck stock|first-time fix|clinic supply|dental distributor/i.test(firstReadText(willow)) &&
      !/ingredient|batch|QA hold|finished-good|case pack/i.test(firstReadText(northstar)) &&
      !/clinic supply|style\/color\/size|dealer allocation/i.test(firstReadText(bayview)) &&
      !/first-time fix|truck stock|clinic supply|dealer allocation/i.test(firstReadText(harbor)),
    JSON.stringify({ willow: firstReadText(willow).slice(0, 1200), northstar: firstReadText(northstar).slice(0, 1200) }));

  assertCase(results, 'w375-collapsed-support-lane-consistency',
    [willow, northstar].every((scenario) => /Lane-consistent support/.test(scenario.runText)) &&
      !/Industrial Distributor|Industrial Distribution|production routing|fashion collection/i.test(supportText(willow)),
    supportText(willow).slice(0, 3600));

  assertCase(results, 'w375-w371-roi-competitive-flow-preserved',
    ...
    willow.valueText.slice(0, 3000));

  assertCase(results, 'w375-w371-run-clickable-open-link-preserved',
  ...
  assertCase(results, 'w375-w374-food-beverage-regression',
    willow.value.foodBeveragePolishW374 &&
      ...
      /Fishbowl|BatchMaster|co-packer|QA hold/i.test(willow.valueText),
    willow.valueText.slice(0, 3600));

  assertCase(results, 'w375-claim-safety-confidence-and-review-trace',
    /Baseline to capture/i.test(willow.valueText) &&
      ...
      /verified_openable|Open links verified|Open/.test(willow.traceText + willow.runText) &&
    ...
    willow.traceText.slice(0, 2400));

  printResults('W375 cross-lane story helper consolidation and fixture-first expansion readiness harness', results);
}
