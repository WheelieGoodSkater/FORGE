...
function main() {
  ...
  assertCase(results, 'w379-expected-proof-role-coverage-visible',
    readinessByLane.dealer_hardgoods.status === 'ready_now' &&
      ...
      readinessByLane.life_sciences.status === 'needs_scoped_source_pack_cleanup',
    JSON.stringify(readiness, null, 2));

  assertCase(results, 'w379-fixture-to-pack-alignment-gaps-not-hidden',
    readiness.filter((entry) => entry.status === 'needs_scoped_source_pack_cleanup').length === 3 &&
      readinessByLane.life_sciences.packIds.length === 0 &&
