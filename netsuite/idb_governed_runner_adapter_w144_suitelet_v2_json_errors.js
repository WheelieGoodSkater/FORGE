...
define(['N/runtime', 'N/task', 'N/log', 'N/file', 'N/search'], (runtime, task, log, file, search) => {
  ...
  function buildRunnerParams(request, config, idempotencyToken) {
    ...
    runnerParams[RUNNER_PARAM_MAP.enableWip] = 'F';
    runnerParams[RUNNER_PARAM_MAP.enableManufacturing] = 'F';
    runnerParams[RUNNER_PARAM_MAP.createNewHero] = 'F';
    runnerParams[RUNNER_PARAM_MAP.heroItem] = '';
    runnerParams[RUNNER_PARAM_MAP.resultCaptureFolderId] = config.resultCaptureFolderId;
    runnerParams[RUNNER_PARAM_MAP.confirmedBuildRequestJson] = JSON.stringify({
  ...
  function resultCapturePending(idempotencyToken, queueSubmit) {
    return {
      schema: 'idb.runner-result-capture.v1',
  ...
  function promotePendingSidecarIfTransactionResolved(pendingSidecar, modules) {
    ...
    const completed = {
      ...
      idempotencyToken: pendingSidecar.partialGeneratedNamesJson && pendingSidecar.partialGeneratedNamesJson.idempotencyToken || '',
      records: {
  ...
  function findResultCaptureFile(config, lookup, modules) {
    ...
    const runnerTaskId = String(lookup.runnerTaskId || '').trim();
    const idempotencyToken = String(lookup.idempotencyToken || '').trim();
    if (!runnerTaskId && !idempotencyToken) return { found: false, reason: 'runnerTaskId or idempotencyToken is required' };
    const searchModule = modules && modules.search;
    ...
    const tokenFilters = [];
    if (runnerTaskId) tokenFilters.push(['name', 'contains', runnerTaskId]);
    if (idempotencyToken) {
      tokenFilters.push(['name', 'contains', idempotencyToken]);
      const safeToken = safeFileToken(idempotencyToken);
      if (safeToken !== idempotencyToken) tokenFilters.push(['name', 'contains', safeToken]);
    }
    let nameFilter = tokenFilters[0];
    for (let i = 1; i < tokenFilters.length; i++) {
      nameFilter = [nameFilter, 'OR', tokenFilters[i]];
    }
    const filters = nameFilter
      ? [['folder', 'anyof', config.resultCaptureFolderId], 'AND', nameFilter]
      : [['folder', 'anyof', config.resultCaptureFolderId]];
    const captureSearch = searchModule.create({
      type: 'file',
      filters,
      columns: ['internalid', 'name', 'modified']
    });
    const matches = captureSearch.run().getRange({ start: 0, end: 5 }) || [];
    if (!matches.length) return { found: false, reason: 'result capture file not found', filters };
    const match = matches[0];
    const fileId = match.id || match.getValue && match.getValue({ name: 'internalid' });
  ...
  function buildResultCapturePollEnvelope(lookup, config, modules, parseErrors) {
    const runnerTaskId = String(lookup && lookup.runnerTaskId || '').trim();
    const idempotencyToken = String(lookup && lookup.idempotencyToken || '').trim();
  ...
  function buildAdapterResult(request, config, operatorGate, parseErrors) {
    ...
    return {
      ...
      queueGate,
      queueSubmit,
      resultCapture: resultCapturePending(idempotencyToken, queueSubmit),
      noSubmitRollback: {
        supported: true,
