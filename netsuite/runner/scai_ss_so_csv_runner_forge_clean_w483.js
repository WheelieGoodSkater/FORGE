 ...
 *
 * v1.12.7
 * - Adds discovery for authoritative precomputed naming files using extId when the explicit naming file parameter is missing or unavailable.
 * - Hardens precomputed naming application so record names are never generated by runner-side fallback text.
 ...
 * - Loads the naming payload from a file-cabinet JSON file keyed to the run and stops if the authoritative pack is missing or incomplete.
 * - Keeps runner execution pack-authoritative, faster, and easier to debug.
 *
 * v1.12.3
...
define(['N/runtime', 'N/log', 'N/search', 'N/record', 'N/https', 'N/task', 'N/file'],
  ...
  function executeMain() {
    ...
    log.audit({
      ...
      details: JSON.stringify({
        ...
        namingSourceUsed: namingPayload.source || names._source || 'authoritative-pack',
  ...
  function buildReturnedRecordsW483(args) {
    ...
    records.heroItem = normalizeSidecarRecordW483({
      ...
      name: authoritativeProductNameW483(names),
      id: ids.heroItemId
    });
    ...
    if (args.enableManufacturing) {
      ...
      records.bomRevision = normalizeSidecarRecordW483({
        ...
        type: 'bomrevision',
        label: 'BOM revision',
        name: names.bom_revision_name,
        id: ids.bomRevId
      });
      const componentNames = Array.isArray(names.component_names) ? names.component_names : [];
      [
        { id: ids.comp1Id, name: componentNames[0] },
        { id: ids.comp2Id, name: componentNames[1] },
        { id: ids.comp3Id, name: componentNames[2] }
      ...
      if (args.woId) {
        records.workOrder = normalizeSidecarRecordW483({
          ...
          type: 'workorder',
          label: 'Work Order',
          name: `WO - ${names.assembly_name || authoritativeProduct}`,
          id: args.woId
        });
  ...
  function createAndAttachRoutingIfPossible({ subsidiaryId, locationId, bomId, assemblyId, extId, prospect, signalText, workCenterSearchId, names }) {
    ...
    // Step 2: Use scenario-aware routing names from the authoritative upstream naming pack.
    ...
    const routingName = trimLen(names && names.routing_name, 80).slice(0, 60);
    ...
    addStep(10, opNames.op10, c1.id, t1.id);
    addStep(20, opNames.op20, c2.id, t2.id);
    addStep(30, opNames.op30, c3.id, t3.id);
  ...
  function resolveRoutingNames({ prospect, signalText, names }) {
    ...
    if (bySeq) {
      return {
        op10: trimLen(str(bySeq['10'] || bySeq[10]), 60),
        op20: trimLen(str(bySeq['20'] || bySeq[20]), 60),
        op30: trimLen(str(bySeq['30'] || bySeq[30]), 60)
      };
    }
    return {
      op10: '',
  ...
  function websiteHeroNameForCreateW483(names, prospect) {
    return authoritativeProductNameW483(names);
  }
  ...
  // ----------------------------
  // Authoritative precomputed naming payload
  // ----------------------------
  function loadPrecomputedNamingPack({ fileId, extId, prospect, website, signalText }) {
    ...
    if (!candidateFileId) {
      return {
        ...
        fileId: null,
        discoveryMode,
        source: 'missing-authoritative-naming-pack',
        payload: emptyAuthoritativeNamingPackW508({ signalText, reason: 'missing naming file' })
      };
    }
    ...
    try {
      ...
      out.component_names = Array.isArray(out.component_names) ? out.component_names : [];
      ...
      log.error({ title: `Precomputed naming load FAILED (authoritative pack required) [${VERSION}]`, details: JSON.stringify({ fileId: candidateFileId, extId: extId || '', message: (e && (e.message || e.details)) ? String(e.message || e.details) : String(e) }) });
      return {
        found: false,
        parsed: false,
        applied: false,
        ...
        discoveryMode,
        source: 'failed-authoritative-naming-pack',
        payload: emptyAuthoritativeNamingPackW508({ signalText, reason: 'failed naming file load' })
      };
    }
  }
  ...
  function emptyAuthoritativeNamingPackW508({ signalText, reason }) {
    return {
      _source: 'missing-authoritative-naming-pack',
  ...
  function assertAuthoritativeNamingPackW508(names, opts) {
    const missing = [];
    const manufacturing = !!(opts && opts.enableManufacturing);
    const wip = !!(opts && opts.enableWip);
    const ops = names && names.operation_names_by_seq || {};
    const components = Array.isArray(names && names.component_names) ? names.component_names.map(compactText).filter(Boolean) : [];
    if (!authoritativeProductNameW483(names)) missing.push('hero_item_name');
    if (manufacturing) {
      if (!compactText(names && names.assembly_name)) missing.push('assembly_name');
      if (components.length !== 3) missing.push('component_names[3]');
      if (!compactText(names && names.bom_name)) missing.push('bom_name');
      if (!compactText(names && names.bom_revision_name)) missing.push('bom_revision_name');
    }
    if (wip) {
      if (!compactText(names && names.routing_name)) missing.push('routing_name');
      if (!compactText(ops['10'] || ops[10])) missing.push('operation_names_by_seq.10');
      if (!compactText(ops['20'] || ops[20])) missing.push('operation_names_by_seq.20');
      if (!compactText(ops['30'] || ops[30])) missing.push('operation_names_by_seq.30');
    }
    if (!missing.length) return;
    const err = new Error(`Authoritative naming pack incomplete: ${missing.join(', ')}`);
    err.name = 'AUTHORITATIVE_NAMING_PACK_REQUIRED';
    err.details = JSON.stringify({ missing, source: names && (names._source || names.namingEvidenceSource) || '' });
    throw err;
  }

  function generateNamingPack({ prospect, website, signalText }) {
    return emptyAuthoritativeNamingPackW508({ signalText, reason: 'authoritative naming pack missing' });
  }
  ...
  function applyNamingToAnchors(ids, names, opts) {
    ...
    const authoritativeProduct = authoritativeProductNameW483(names);
