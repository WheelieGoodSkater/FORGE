...
define(['N/runtime', 'N/log', 'N/search', 'N/record', 'N/https', 'N/task', 'N/file'],
  ...
  function execute() {
    ...
    const prospect = str(s.getParameter({ name: 'custscript_scai_so_runner_prospect' }));
    const website  = str(s.getParameter({ name: 'custscript_scai_so_runner_website' }));
    const notes    = str(s.getParameter({ name: 'custscript_scai_so_runner_notes' }));
    const agenda   = str(s.getParameter({ name: 'custscript_scai_so_runner_agenda' }));
    const extId    = str(s.getParameter({ name: 'custscript_scai_so_runner_extid' }));

    const soMappingId = toIntOrNull(s.getParameter({ name: 'custscript_scai_so_runner_mapping' }));
    const soFolderId  = toIntOrNull(s.getParameter({ name: 'custscript_scai_so_runner_folder' }));
    const namingFileId = toIntOrNull(s.getParameter({ name: 'custscript_scai_runner_naming_file_id' }));

    const subsidiaryId = toIntOrNull(s.getParameter({ name: 'custscript_scai_runner_subsidiary' }));
    const locationId   = toIntOrNull(s.getParameter({ name: 'custscript_scai_runner_location' }));
    const workCenterSearchIdRaw = str(
      s.getParameter({ name: 'custscript_scai_wc_savedsearch_id' }) ||
      s.getParameter({ name: 'custscript_scai_runner_wc_search' })
    ).trim();
    ...
    const enableWipRaw =
      s.getParameter({ name: 'custscript_scai_runner_enable_wip' }) ||
      s.getParameter({ name: 'custscript_scai_runner_enablewip' });
