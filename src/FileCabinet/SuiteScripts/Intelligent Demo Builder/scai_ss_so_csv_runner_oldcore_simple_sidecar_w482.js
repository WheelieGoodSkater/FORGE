...
define(['N/runtime', 'N/log', 'N/search', 'N/record', 'N/https', 'N/task', 'N/file'],
   ...
   */
  const VERSION = 'v1.12.13-w482-simple-sidecar';
  const RUNNER_EXECUTION_CORE_W482 = 'old-runner-v1.12.13';
  const SIDECAR_VERSION_W482 = 'w482-oldcore-simple-sidecar';
  const RESULT_CAPTURE_FILENAME_LIMIT_W482 = 180;
  ...
  function execute() {
    ...
    const prospect = str(getScriptParamAny(s, ['custscript_w482_prospect', 'custscript_v3_runner_prospect', 'custscript_scai_so_runner_prospect']));
    const website  = str(getScriptParamAny(s, ['custscript_w482_website', 'custscript_v3_runner_website', 'custscript_scai_so_runner_website']));
    const notes    = str(getScriptParamAny(s, ['custscript_w482_notes', 'custscript_v3_runner_notes', 'custscript_scai_so_runner_notes']));
    const agenda   = str(getScriptParamAny(s, ['custscript_w482_agenda', 'custscript_v3_runner_agenda', 'custscript_scai_so_runner_agenda']));
    const extId    = str(getScriptParamAny(s, ['custscript_w482_extid', 'custscript_v3_runner_extid', 'custscript_scai_so_runner_extid']));

    const soMappingId = toIntOrNull(getScriptParamAny(s, ['custscript_w482_mapping', 'custscript_v3_runner_mapping', 'custscript_scai_so_runner_mapping']));
    const soFolderId  = toIntOrNull(getScriptParamAny(s, ['custscript_w482_folder', 'custscript_v3_runner_folder', 'custscript_scai_so_runner_folder']));
    const namingFileId = toIntOrNull(getScriptParamAny(s, ['custscript_w482_naming_file', 'custscript_scai_runner_naming_file_id']));
    const resultCaptureFolderId = toIntOrNull(getScriptParamAny(s, ['custscript_w482_result_folder', 'custscript_v3_runner_result_capture_folder', 'custscript_idb_result_capture_folder_id']));
    const confirmedBuildRequestJson = safeJsonParse(getScriptParamAny(s, ['custscript_w482_req_json', 'custscript_v3_runner_idb_request_json'])) || {};

    const subsidiaryId = toIntOrNull(getScriptParamAny(s, ['custscript_w482_subsidiary', 'custscript_v3_runner_subsidiary', 'custscript_scai_runner_subsidiary']));
    const locationId   = toIntOrNull(getScriptParamAny(s, ['custscript_w482_location', 'custscript_v3_runner_location', 'custscript_scai_runner_location']));
    const workCenterSearchIdRaw = str(
      getScriptParamAny(s, ['custscript_w482_wc_search', 'custscript_v3_runner_wc_search', 'custscript_scai_wc_savedsearch_id', 'custscript_scai_runner_wc_search'])
    ).trim();
    ...
    const enableWipRaw = getScriptParamAny(s, ['custscript_w482_enable_wip', 'custscript_v3_runner_enable_wip', 'custscript_scai_runner_enable_wip', 'custscript_scai_runner_enablewip']);
    const enableWip = normalizeBool(enableWipRaw);

    const createNewHeroCandidates = {
      custscript_w482_create_hero: s.getParameter({ name: 'custscript_w482_create_hero' }),
    ...
    const enableManufacturingCandidates = {
      custscript_w482_enable_mfg: s.getParameter({ name: 'custscript_w482_enable_mfg' }),
    ...
    const passedHeroItemIdRaw =
      s.getParameter({ name: 'custscript_w482_hero_item' }) ||
    ...
    const sidecarCapture = writeForgeSidecarResultW482({
  ...
  function writeForgeSidecarResultW482(args) {
    ...
    const completedResult = {
      ...
      sidecarVersion: SIDECAR_VERSION_W482,
      runnerExecutionCore: RUNNER_EXECUTION_CORE_W482,
      status,
      runStatus: status,
      ...
      productBuildPlanW482: buildProductBuildPlanW482(args),
    ...
    const saved = saveTextArtifactW482({
      folderId,
      name: resultCaptureFileNameW482({ extId, buildAttemptId, status }),
  ...
  function buildReturnedRecordsW482(args) {
    const ids = args.ids || {};
    const names = args.names || {};
    const records = {};
    const customerId = findByExternalId('customer', ANCHORS.customer);
    records.customer = normalizeSidecarRecordW482({
      role: 'customer',
      type: 'customer',
      label: 'Customer',
    ...
    records.demoTransaction = normalizeSidecarRecordW482({
      role: 'sales_order',
      type: 'salesorder',
      label: 'Sales Order',
    ...
    records.heroItem = normalizeSidecarRecordW482({
      role: 'hero_item',
      type: 'inventoryitem',
    ...
    if (args.enableManufacturing) {
      records.assembly = normalizeSidecarRecordW482({
        role: 'assembly',
        type: 'assemblyitem',
        label: 'Assembly',
      ...
      records.bom = normalizeSidecarRecordW482({
        role: 'bom',
        type: 'bom',
        label: 'BOM',
      ...
      records.bomRevision = normalizeSidecarRecordW482({
        role: 'bom_revision',
        type: 'bomrevision',
        label: 'BOM revision',
      ...
      [
        ...
        records[`componentItem${index + 1}`] = normalizeSidecarRecordW482({
          role: 'component_item',
          type: 'inventoryitem',
      ...
      if (args.woId) {
        records.workOrder = normalizeSidecarRecordW482({
          role: 'work_order',
          type: 'workorder',
          label: 'Work Order',
      ...
      if (args.routingId) {
        records.routing = normalizeSidecarRecordW482({
          role: 'routing',
  ...
  function normalizeSidecarRecordW482(input) {
    const id = input && input.id ? String(input.id) : '';
    const type = input && input.type || '';
    const plannedOnly = !!(input && input.plannedOnly);
    const url = id && !plannedOnly ? recordUrlW482(type, id) : '';
    return {
  ...
  function recordUrlW482(type, id) {
    ...
    return `${netsuiteOriginW482()}${path}?id=${encodeURIComponent(internalId)}`;
  }

  function netsuiteOriginW482() {
    const acct = String(runtime.accountId || '').replace(/_/g, '-').toLowerCase();
    return `https://${acct || 'system'}.app.netsuite.com`;
  }

  function buildProductBuildPlanW482(args) {
    const names = args.names || {};
    return {
      schema: 'idb.product-build-plan.w482.old-runner-naming.v1',
  ...
  function buildRoiCompetitiveSidecarW482(args, records) {
    const confirmed = args && args.confirmedBuildRequestJson || {};
    const storyInputs = confirmed.storyInputs || {};
    const names = args.names || {};
    const selectedProduct = firstNonBlankTextW482(
    ...
    const painSignal = firstNonBlankTextW482(storyInputs.pain, storyInputs.buyerPain, extractPainSignalW482(notes), storyInputs.buyerNeed);
    const baseline = firstNonBlankTextW482(storyInputs.buyerBaseline, confirmed.buyerBaseline, confirmed.roiBaseline);
    const competitors = competitorTokensW482([
      ...
      confirmed.incumbent,
      extractCompetitorEvidenceW482(notes)
    ]);
    ...
    const sourceBasis = uniqueTextValuesW482([
    ...
    const valueReviewPacket = confirmed.roiCompetitiveReview || {
      schema: 'idb.w482-runner-sidecar-value-review.v1',
    ...
    const roiAudit = Object.assign({
      schema: 'idb.w482-runner-roi-audit.v1',
      advisoryOnly: true,
      claim: roiClaim,
      metricDirection,
    ...
    const competitive = Object.assign({
      schema: 'idb.w482-runner-competitive.v1',
      advisoryOnly: true,
    ...
    const competitiveAdvisory = Object.assign({
      ...
      sidecarSchema: 'idb.w482-runner-competitive-advisory.v1',
    ...
    const detailModel = Object.assign({
      schema: 'idb.w482-runner-roi-competitive-detail-model.v1',
      roi: {
        source: 'runner_simple_advisory',
        confidence,
    ...
    return {
      schema: 'idb.w482-runner-roi-competitive-sidecar.v1',
  ...
  function saveTextArtifactW482(args) {
    const f = file.create({
      name: boundedFileNameW482(args.name || `idb_runner_sidecar_${Date.now()}.json`, RESULT_CAPTURE_FILENAME_LIMIT_W482),
  ...
  function resultCaptureFileNameW482(args) {
    const extId = safeCodeW482(args && args.extId || 'idb');
    const attempt = safeCodeW482(args && args.buildAttemptId || '').slice(0, 48);
    const status = safeCodeW482(args && args.status || 'result');
    const stem = attempt || extId || status || 'idb';
    return boundedFileNameW482(`idb_runner_sidecar_${status}_${stem}_${extId}.json`, RESULT_CAPTURE_FILENAME_LIMIT_W482);
  }

  function boundedFileNameW482(name, maxLen) {
  ...
  function extractPainSignalW482(notes) {
  ...
  function extractCompetitorEvidenceW482(notes) {
  ...
  function competitorTokensW482(values) {
    const out = [];
    ...
    return uniqueTextValuesW482(out).slice(0, 5);
  }

  function uniqueTextValuesW482(values) {
    const seen = {};
    const out = [];
    (values || []).forEach(function(value) {
